...
class imuBiasCompensatorNode : public rclcpp::Node {
 public:
  imuBiasCompensatorNode() : Node("imu_bias_compensator_node") {
    biasSub = this->create_subscription<geometry_msgs::msg::Vector3Stamped>(
        "bias_topic_in", 10, std::bind(&imuBiasCompensatorNode::biasMsgCallback, this, std::placeholders::_1));
 ...
 private:
  ...
  void biasMsgCallback(const geometry_msgs::msg::Vector3Stamped& biasMsg) {
    ...
    imuSubscription = this->create_subscription<sensor_msgs::msg::Imu>(
        "imu_topic_in", 10, std::bind(&imuBiasCompensatorNode::imuMsgCallback, this, std::placeholders::_1));
  }
